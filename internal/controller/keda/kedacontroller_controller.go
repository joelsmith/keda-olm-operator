...
func (r *KedaControllerReconciler) installGeneralResources(ctx context.Context, logger logr.Logger, instance *kedav1alpha1.KedaController) error {
	...
	r.resourcesGeneral = r.resourcesGeneral.Filter(mf.Not(isEmptyKedaAllowEgressToAllNetworkPolicy))

	if err := toDelete.Delete(); err != nil {
		logger.Info("Unable to delete unwanted NetworkPolicy keda-allow-egress-to-all", "error", err)
		return err
	}
		...
		if u.GetKind() == "NetworkPolicy" {
			...
			if policy.Name == "keda-allow-egress-to-all" {
				...
				for _, expr := range matchExprs {
					if expr.Key == "name" && expr.Operator == metav1.LabelSelectorOpIn && len(expr.Values) == 0 {
						emptyName = true
					}
